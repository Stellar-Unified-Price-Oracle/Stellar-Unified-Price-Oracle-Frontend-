...
// ─── Error states ─────────────────────────────────────────────────────────────
// The build under test has MSW enabled, so its service worker intercepts
// /api/* before Playwright's page.route does. These three tests must block
// service workers so the route mocks below actually take effect.
...
test('clicking the site logo navigates to the landing page', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Stellar Unified Price Oracle' })).toBeVisible({ timeout: 10_000 })
})
...
test('navigating back from 404 to dashboard works', async ({ page }) => {
  ...
  // Use the 404 page's explicit "Back to Dashboard" link (the logo goes to
  // the Landing page at "/").
...
test('search query is reflected in the URL', async ({ page }) => {
  ...
  await page.getByRole('combobox', { name: 'Search asset pairs' }).fill('BTC')
...
test('reloading page with search param preserves search state', async ({ page }) => {
  await page.goto('/dashboard?search=ETH')
  await page.waitForLoadState('networkidle')
