...
export interface AlertInput {
  ...
  percentageDirection?: 'up' | 'down' | 'either'
  percentageWindow?: '5min' | '15min' | '1hr' | '24hr'
}
    ...
    const next = this.queue.then(async () => {
      while (true) {
        ...
        this.tokens = Math.min(
          ...
          this.tokens + ((now - this.updatedAt) / 1000) * this.options.refillPerSecond,
        )
        this.updatedAt = now
        if (this.tokens >= 1) {
          this.tokens -= 1
        ...
        await defaultSleep(Math.ceil(((1 - this.tokens) / this.options.refillPerSecond) * 1000))
      }
    })
...
export class OracleClient {
  ...
  constructor(options: OracleClientOptions = {}) {
    ...
    this.jitterRatio = Math.max(0, options.jitterRatio ?? 0.2)
    this.sleep = options.sleep ?? defaultSleep
    this.random = options.random ?? Math.random
    this.limiter = options.limiter ? new TokenBucket(options.limiter) : undefined
    if (options.onRateLimitChange) this.rateLimitListeners.add(options.onRateLimitChange)
  }
  ...
  async getPrice(pair: string): Promise<PriceData> {
    return this.request<PriceData>(`/api/prices/${encodeURIComponent(pair)}`)
  }
  async getPrices(): Promise<PriceData[]> {
    return this.request<PriceData[]>('/api/prices')
  }
  async getHistory(pair: string, limit = 100, offset = 0): Promise<PriceHistoryResponse> {
  ...
  subscribe(pairs: string[], onPrice: (price: PriceData) => void): () => void {
    ...
    const open = () => socket.send(JSON.stringify({ type: 'subscribe', pairs }))
    const message = (event: MessageEvent<string>) => {
      try {
        const data: unknown = JSON.parse(event.data)
        if (typeof data === 'object' && data !== null && 'type' in data && data.type === 'price_update')
          onPrice(data as unknown as PriceData)
      } catch {
  ...
  private captureRateLimit(response: Response): void {
    ...
    response.headers.forEach((value, key) => {
      if (key.toLowerCase().startsWith('x-ratelimit-') || key.toLowerCase() === 'retry-after') headers[key] = value
    })
