...
export function RequireWallet({ children }: { children: ReactNode }): ReactElement {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-gray-300 dark:border-gray-700 px-6 py-10 text-center">
      <p className="text-sm font-medium text-gray-700 dark:text-gray-200">{t('wallet.gate.title')}</p>
      <p className="text-sm text-gray-500 dark:text-gray-400 max-w-sm">{t('wallet.gate.description')}</p>
      <button
        type="button"
        onClick={() => void connect()}
        disabled={status === 'connecting'}
        className="mt-1 px-4 py-2 rounded-lg text-sm font-medium bg-cyan-700 text-white hover:bg-cyan-600 disabled:opacity-60 transition-colors"
      >
